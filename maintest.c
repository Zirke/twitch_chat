...
void read_data_log(int *total_entries_log, chat_entry logs[]);
...
int main(void){
    ...
    int total_entries_wordlist = 0;
    int total_entries_log = 1;
    ...
    read_wordlist(&total_entries_wordlist, words);
    read_data_log(&total_entries_log, logs);
...
void read_data_log(int *total_entries_log, chat_entry logs[]){
  ...
  *total_entries_log = countAllEntries(chat_log);
  logs = realloc(logs, sizeof(chat_entry) * (*total_entries_log));
  chat_entry data = {0};
...
void read_wordlist(int *total_entries_wordlist, wordlist words[]){
  ...
  *total_entries_wordlist = countAllEntries(user_wordlist);
  words = realloc(words, sizeof(wordlist) * (*total_entries_wordlist));
  wordlist data = {0};
